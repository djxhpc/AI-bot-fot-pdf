...
def _create_parent_chunks(docs):
    """
    Parent chunks (1500 chars): keep full context for LLM to answer from.
...
def _create_child_chunks(parent_doc):
    """
    Child chunks (300 chars): high-precision retrieval index only, NOT fed to LLM.
    Inherits parent metadata and stores parent_id for look-up.
    """
    from langchain_text_splitters import RecursiveCharacterTextSplitter
    splitter = RecursiveCharacterTextSplitter(
        chunk_size=300,
...
", "\u3002", ".", "\uff1b", ";", "\uff0c", ",", " ", ""],
    ...
    children = splitter.create_documents([parent_doc.page_content])
    for child in children:
        child.metadata = {**parent_doc.metadata}
    return children
...
def build_vector_store(docs, summarize_model: str = ""):
    ...
    # --- Parent chunks (large, for LLM reading) ---
    parent_docs = _create_parent_chunks(docs)
    for p in parent_docs:
        p.metadata["doc_context"] = global_meta_info or "\u901a\u7528\u6587\u672c"
        p.metadata["parent_id"]   = str(uuid.uuid4())

    if global_meta_info:
        meta_pid = str(uuid.uuid4())
        meta_parent = Document(
            page_content=f"\u672c\u6587\u4ef6\u57fa\u672c\u8cc7\u8a0a\uff1a{global_meta_info}\u3002\u672c\u624b\u518a\u70ba\u516c\u53f8\u5de5\u4f5c\u898f\u5247\u3002",
            metadata={"page": 0, "parent_id": meta_pid, "doc_context": global_meta_info},
        )
        all_parents = [meta_parent] + parent_docs
    else:
        all_parents = parent_docs

    # parent_id -> parent_doc
    id_to_parent = {p.metadata["parent_id"]: p for p in all_parents}

    # --- Child chunks (small, for vector index) ---
    all_children = []
    for parent in all_parents:
        children = _create_child_chunks(parent)
        for child in children:
    ...
    with st.spinner(
        f"\u6b63\u5728\u5efa\u7acb Parent-Child \u7d22\u5f15\u2026"
        f"\u7236\u6bb5\u843d {len(all_parents)} \u500b / "
        f"\u5b50\u6bb5\u843d {len(all_children)} \u500b"
    ):
        vector_db = FAISS.from_documents(all_children, get_embedding_model())
        try:
            import jieba
        ...
        with open("faiss_index_storage/sections.pkl", "wb") as f:
            pickle.dump(all_children, f)        # BM25 rebuild (children)
        with open("faiss_index_storage/id_to_full.pkl", "wb") as f:
            pickle.dump(id_to_parent, f)        # parent_id -> parent_doc
    ...
    return {
        ...
        "id_to_full":     id_to_parent,   # key reused; now maps parent_id -> parent_doc
    }
...
def get_relevant_context(query, hybrid_db, k_value):
    ...
    # 3. Dedup by parent_id; look up the parent doc for full context.
    #    Fallback: if parent_id missing (old DB), use doc_id instead.
    seen_ids = set()
    full_text_candidates = []
    for child_doc in dense_summary_docs + sparse_summary_docs:
        parent_id = child_doc.metadata.get("parent_id")
        lookup_id = parent_id or child_doc.metadata.get("doc_id")
        if lookup_id and lookup_id not in seen_ids:
            seen_ids.add(lookup_id)
            parent_doc = id_to_full.get(lookup_id, child_doc)
            full_text_candidates.append(parent_doc)
    ...
    # 2. Force-include meta parent for company-info queries
    if any(kwd in query for kwd in ["\u54ea\u9593", "\u516c\u53f8", "\u54ea\u5bb6", "\u4ec0\u9ebc\u6587\u4ef6", "\u662f\u8ab0\u7684", "\u5b88\u5247"]):
        for parent in id_to_full.values():
            if "\u672c\u6587\u4ef6\u57fa\u672c\u8cc7\u8a0a\uff1a" in parent.page_content:
                if parent not in final_docs:
                    final_docs.insert(0, parent)
                break
...
def get_rag_answer(question: str, model_name: str, k: int) -> tuple[str, str]:
    ...
    system_instruction = (
        ...
        "8. 【數字門檻嚴格比對】：當條文列出明確的數字條件（年資、年齡、天數、金額等），必須逐一核對問題中的數字是否達標。"
        "若未達任何一個條件，必須明確回答「不符合」，絕對不可說「符合」。"
    )
...
def _judge_answer(question: str, system_answer: str, expected_answer: str,
                  context: str, model_name: str) -> dict:
    """
    LLM-as-Judge：用 LLM 評估系統回答的品質。
    回傳 {"verdict": "PASS"/"FAIL"/"PARTIAL", "reason": str, "hallucination": bool}
    """
    judge_prompt = f"""你是一個嚴格的評審，請根據以下資訊評估「系統回答」的品質。
...
請依序回答以下三點（每點一行，格式固定）：
1. 判決: PASS 或 FAIL 或 PARTIAL（PASS=系統回答與預期一致；PARTIAL=部分正確；FAIL=錯誤或幻覺）
2. 幻覺: YES 或 NO（系統回答是否包含手冊原文以外的資訊或錯誤推論）
3. 理由: 一句話說明判決原因
...
只輸出這三行，不要其他說明。"""
    try:
        ...
        reason       = raw
        for line in lines:
            if line.startswith("1.") or line.startswith("判決"):
                if   "PASS"    in line.upper(): verdict = "PASS"
                elif "PARTIAL" in line.upper(): verdict = "PARTIAL"
                else:                           verdict = "FAIL"
            elif line.startswith("2.") or line.startswith("幻覺"):
                hallucination = "YES" in line.upper()
            elif line.startswith("3.") or line.startswith("理由"):
                reason = line.split(":", 1)[-1].strip()
        return {"verdict": verdict, "hallucination": hallucination, "reason": reason}
    except Exception as e:
        return {"verdict": "ERROR", "hallucination": False, "reason": str(e)}
        ...
        with st.chat_message("assistant"):
            ...
            # Rerank 門檻攔截：知識庫無相關內容
                ...
                system_instruction = (
                    ...
                    "8. 【數字門檻嚴格比對】：當條文列出明確的數字條件（年資、年齡、天數、金額等），必須逐一核對問題中的數字是否達標。"
                    "若未達任何一個條件，必須明確回答「不符合」，絕對不可說「符合」。\n"
                    "9. 遇到打招呼，請親切回應，無需引用手冊。\n"
                    "10. 遇到無意義亂碼，請禮貌說明並引導詢問手冊相關問題。"
                )

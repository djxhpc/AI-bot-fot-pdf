        ...
        with st.chat_message("assistant"):
            ...
            # Rerank 門檻攔截：知識庫無相關內容
                ...
                try:
                    ...
                    # ========== Hallucination 校驗（後處理）==========
                    if has_any_db and context != "__NO_RELEVANT_CONTEXT__":
                        verify_prompt = f"""請檢查以下回答是否只引用自手冊內容，有無捏造數字、日期或細節。
                ...
                回覆規則：
                - 若無捏造，回覆「✅ 無問題」
                - 若有捏造，請明確說明哪些部分是手冊未記載的，不要只說「部分資訊存疑」"""
                        ...
                        try:
                            for v_chunk in chat(
                                ...
                                verify_response += v_chunk.message.content
                        except Exception:
                            verify_response = ""  # 校驗失敗不影響主流程

                        if "✅" not in verify_response:
                            answer = answer + f"\n\n⚠️ **此回答可能包含手冊未記載的資訊(幻覺產生)，請自行查證。**"
